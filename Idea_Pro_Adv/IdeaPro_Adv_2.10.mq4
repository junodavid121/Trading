...
#property description "Change entry point same as auto grid"
...
bool determineOrder(int period, int type)
{
  ...
  //Buy if Ask price below MA line
    if(!SelectLastCurrentTrade(OP_BUY) && Cross(2, (iMA(NULL, PERIOD_CURRENT, period, 1, MODE_SMA, PRICE_CLOSE, 0)-Ask)>startOrderDis*0.00001) && OrderOpenPrice()- Ask >= 0.00001*Step && Ask - iLow(Symbol(),PERIOD_H1,0) >= 0.00001*AutoGrid)
    {
      return true;
    }
...
double determineClose(int period, int type)
{
  ...
  if(SelectLastCurrentTrade(type) && currentTotalProfit > setProfit)
  {
   return true;
  }
