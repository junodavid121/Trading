...
#property description "Changed Auto Grid Timeframe to H4"
...
bool determineOrder(int period, int type)
{
  ...
  //Buy if Ask price below MA line
    ...
    {
      if(OrderOpenPrice()- Ask >= 0.00001*Step && Ask - iLow(Symbol(),PERIOD_H4,0) >= 0.00001*AutoGrid)
      {
        return true;
      }
    }
  ...
  else if(type == OP_SELL)
  {
    ...
    {
      if( Bid - OrderOpenPrice() >= 0.00001*Step && iHigh(Symbol(),PERIOD_H4,0) - Bid >= 0.00001*AutoGrid)
      {
        return true;
      }
    }
