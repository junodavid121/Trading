...
bool determineOrder(int type)
{
  ...
  //Buy if Ask price below MA line
    ...
    {
      if(OrderOpenPrice()- Ask >= 0.00001*200)
      {
        return true;
      }
    }
  ...
  else if(type == OP_SELL)
  {
    ...
    {
      if( Bid - OrderOpenPrice() >= 0.00001*200)
      {
        return true;
      }
    }
...
void OnTick()
  {
   ...
   if( num > 5)
   {
      // check if need to do a buy order
      buyTrade = determineOrder(OP_BUY);
   }
   else if( num <= 5)
   {
      // check if need to do a sell order
      sellTrade = determineOrder(OP_SELL);
   }
   
   //Check if we need to close all orders
