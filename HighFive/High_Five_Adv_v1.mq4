...
double MM_Size() 
  {
   double lots = OrderSetList[findTotalOrders()-1][0];
   return(lots);
  }
...
double findTotalProfitPoint()
{
  ...
  for(int cnt = 0; cnt <= OrdersTotal(); cnt++){
    ...
    Total_profit += OrderProfit()/Point + OrderSwap()/Point + OrderCommission(/Point);
  }
  return Total_profit;
}
...
bool determineOrder(int type)
{
  ...
  {
  //Buy if num <= 5
    if(!SelectLastCurrentTrade(OP_BUY) && num <= 5 )
    {
      return true;
    }
    ...
    {
      if(findTotalProfitPoint() < -OrderSetList[findTotalOrders()-1][1]*Point)
      {
        return true;
      }
    }
  ...
  {
    //Sell if num > 5
    if(!SelectLastCurrentTrade(OP_SELL) && num  > 5)
    {
      return true;
    }
...
double determineClose()
{
  ...
  double currentTotalProfitPoint = findTotalProfitPoint();
  double currentTotalProfit = findTotalProfit();
  int takeProfit = TakeProfitPoint;

  SelectLastCurrentTrade(type);
  if(findTotalOrders() >= 8)
  {
    takeProfit = 0;
  }
  if(currentTotalProfitPoint > takeProfit*Point)
  {
   return true;
  }
...
int OnInit()
  ...
  //initialize LotDigits
   ...
   OrderSetList[0][0] = 0.01；
   OrderSetList[1][0] = 0.01；
   OrderSetList[2][0] = 0.02；
   OrderSetList[3][0] = 0.03；
   OrderSetList[4][0] = 0.05；
   OrderSetList[5][0] = 0.08；
   OrderSetList[6][0] = 0.13；
   OrderSetList[7][0] = 0.13；
   OrderSetList[8][0] = 0.13；
   OrderSetList[9][0] = 0.13；
   ...
   if(buyTrade) //Moving Average crosses above Moving Average
     {
      ...
      if(IsTradeAllowed())
        {
         ticket1 = myOrderSend("EURUSD",OP_BUY, price, MM_Size(), "");
         ticket2 = myOrderSend("AUDUSD",OP_SELL, price, MM_Size(), "");
         ticket3 = myOrderSend("EURJPY",OP_SELL, price, MM_Size(), "");
         ticket4 = myOrderSend("NZDUSD",OP_SELL, price, MM_Size(), "");
         ticket5 = myOrderSend("USDJPY",OP_SELL, price, MM_Size(), "");
