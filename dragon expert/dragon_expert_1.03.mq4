...
extern double Profit_First = 8;  //in pips
extern double Profit_After = 1;
extern double Breakeven_Order = 10;
extern double Martin_factor = 1.62;
...
int auto_profit(int direction)
   {
      if(TradesCount(direction) == 1)
      {
         return Profit_First;
      }
      else if(TradesCount(direction) > 1)
      {
         return (Profit_First -  TradesCount(direction) * Profit_After);
      }
      return 0;
   }
   ...
   for(int i = 0; i < total; i++)   
     {
      ...
      // Short direction skip buy orders, Long direction skip sell orders.
...
//add buy Matrin order
   ...
   if(Bid - PriceAverageLong() >= auto_profit(OP_BUY) * myPoint //Total Open Profit (Long) >= fixed value
     )
     {
      if(IsTradeAllowed())
  ...
  //Open sell Order
   ...
   if(PriceAverageShort() - Ask >= auto_profit(OP_SELL) * myPoint //Total Open Profit (Long) >= fixed value
     )
     {
      if(IsTradeAllowed())
